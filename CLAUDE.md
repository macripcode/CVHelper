...
**Backend is a single Hono app**: `apps/api/src/index.ts` mounts one router per module under `routes/` (`candidate.ts` — GET only, `vacancy.ts`, `tailor.ts`, `pdf.ts`, `latex.ts`), each backed by services in `services/` (`storage.ts` for all file I/O, `cvTemplate.ts`, `pdfRenderer.ts`, `latexTemplate.ts`, `latexCompiler.ts`). CORS is restricted to `WEB_ORIGIN` (default `http://localhost:5173`).
